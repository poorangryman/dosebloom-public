...
class DoseBloomRepository(private val database: DoseBloomDatabase) {
    ...
    fun observeMedicines(profile: String): Flow<List<Medicine>> =
        medicines.observeByProfile(profile).map { list -> list.map { it.toDomain() } }

    fun observeAllMedicines(): Flow<List<Medicine>> =
        medicines.observeAll().map { list -> list.map { it.toDomain() } }

    fun observeIntakes(date: String): Flow<List<Intake>> =
        intakes.observeForDate(date).map { list -> list.map { it.toDomain() } }

    fun observeIntakes(from: String, to: String): Flow<List<Intake>> =
        intakes.observeBetween(from, to).map { list -> list.map { it.toDomain() } }
...
enum class IntakeStatus(val storageValue: String) {
    ...
    companion object {
        fun fromStorage(value: String): IntakeStatus = entries.firstOrNull { it.storageValue == value } ?: SKIPPED
    }
}
